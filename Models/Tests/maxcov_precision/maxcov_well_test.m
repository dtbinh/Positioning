...
for rep=1:pref.repetitions
    ...
    %% Maxcov - well
    %tic;
    [xy_centroid_well, xy_well_idx] = well5_test(xy_test(1:pref.N,:,rep), [X(:) Y(:)], F);
    %time(rep) = toc;
    
    %% Market share
    triangles = size(xy_centroid_well,1);
    shares = NaN(triangles, 1);
    for tri=1:triangles
        xy_tri = [xy_test(1:pref.N,:,rep); xy_centroid_well(tri,:)];
    ...
    error_share(rep) = share_max-shares(xy_well_idx);
    error_distance(rep) = pdist([xy_centroid_well(xy_well_idx,:); xy_centroid_well(share_max_idx,:)], 'euclidean');
