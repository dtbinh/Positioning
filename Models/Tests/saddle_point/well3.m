...
allintersectxy = vxy; % [vxy; intersectvdxy];
allF = F_v3xy; % [F_v3xy; F_intersectvdxy];
...
[xys_hdist, combis] = pdist2(xy, allintersectxy, 'euclidean', 'Smallest', 2);
...
%     intersectxy_seperate = (pdist2(xy, intersectxy_all, 'euclidean', 'Smallest', 1) >= r-err1)';
...
for i=sort_dist
    ...
    %end
    scatter( allintersectxy(i,1) , allintersectxy(i,2), 'filled', 'MarkerFaceColor', 'm');
    hold off;
    xlim([-5 5]); ylim([-5 5]);
    colorbar;
...
scale = (allF/F_max)' ./ (xys_hdist / distance_max);
...
% hold on;
% scatter( allintersectxy(i,1) , allintersectxy(i,2), 'k*');
% contour(x, y, distance_std, [xys_hdist(i)/max(distance(:)) xys_hdist(i)/max(distance(:))], 'Color', 'black');
% hold off;
% xlim([-5 5]); ylim([-5 5]);
% set(gca,'ydir', 'normal');
