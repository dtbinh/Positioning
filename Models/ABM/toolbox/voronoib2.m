function [P, C, H, L, B] = voronoib2(xy)
%VORONOIB2
...
%     hold off;
	...
	[~,bidx] = max(abs(BXY_split_mid), [], 2);
	posneg = zeros(size(BXY_split_mid));
	posneg(sub2ind(size(BXY_split_mid), 1:size(BXY_split_mid,1), bidx')) = 1;
	posneg = logical(posneg);
	BXY_split_mid_transpose = BXY_split_mid'; 
	% First term: bidx = 1 is x and bidx = 2 is y. The term changes value to 1 if y or 3 if x.
	% Second term: If BXY_split_mid_transpose is positive then top or right -- gives value so 1.
	bside = ((1-(bidx-1))*2+1) + (BXY_split_mid_transpose(posneg') > 0); % Sum 1=bottom, 2=top, 3=left, 4=right
	%BXY_split_mid(test)
	bsiderelation = [IB' bside]; % First column is firm number, second is boundary.
...
H = hull; % Convex hulls
L = all_split; % Lines
B = bsiderelation; % Points relation with boundary
