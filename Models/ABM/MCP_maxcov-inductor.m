%% Monte Carlo Parameterization -- Maxcov-inductor
% version 0.01
% Jonas K. Sekamane
%
...
%% 1. Introduction
% The maxcov-inductor model does not represent a stationary Markov chain / 
...
%
% The maxcov-inductor model does not contain random components (besides the 
% initial position of firms and the initial endowment of condition/forecast
% rules.
%
% So the Markov chain is deterministic. ...
%
...
% Graphically.
    ...
    parfor run=1:pref.runs
        ...
        pref2.rules = repmat( {'MAXCOV-INDUCTOR'}, 1, pref2.N);
    ...
    % mean eccentricity: The estimate is most volitile when subpopulations are polarized (mu close to 1.5). Slowest converge with few firms (2-3). Converges within 100 iterations. 
    % ENP: Most volitile when many firms (10-12). Converge within 25-100 iterations
...
% sample size checks, then increase the number of repetition to 100, etc.
    ...
    export_mean_representation = table(export_param_fmt(:,1), export_param_fmt(:,2), export_param_fmt(:,3), squeeze(est_mean_representation), squeeze(est_std_mean_representation), squeeze(est_se_mean_representation), NaN(pref.runs,1), NaN(pref.runs,1), NaN(pref.runs,1), NaN(pref.runs,1), SESD_ratio_mean_representation, ...
                      'VariableNames', {'N' 'mu' 'n_ratio' 'MeanEst' 'StdDev' 'StdError' 'Check1_Rhat' 'Check2_Ftest' 'Check3_PowerZero' 'Check4_PowerDiff', 'Check5_SESD'});
    writetable(export_mean_eccentricity, strcat('data/MCP_maxcov-inductor_mean_eccentricity_', char(pref.timestamp, 'yyyyMMdd_HHmmss'), '_i', num2str(pref.iterations), '_psi', num2str(pref.psi), '_b', num2str(pref.burnin), '_r', num2str(pref.repetitions), '.csv'),'Delimiter',',');
    writetable(export_ENP, strcat('data/MCP_maxcov-inductor_ENP_', char(pref.timestamp, 'yyyyMMdd_HHmmss'), '_i', num2str(pref.iterations), '_psi', num2str(pref.iterations), '_b', num2str(pref.burnin), '_r', num2str(pref.repetitions), '.csv'),'Delimiter',',');
    writetable(export_mean_representation, strcat('data/MCP_maxcov-inductor_mean_representation_', char(pref.timestamp, 'yyyyMMdd_HHmmss'), '_i', num2str(pref.iterations), '_psi', num2str(pref.iterations), '_b', num2str(pref.burnin), '_r', num2str(pref.repetitions), '.csv'),'Delimiter',',');
